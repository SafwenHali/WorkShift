    ...
    <div className='shadow-lg w-full fixed top-0 left-0'><SigninModal visible={visible} onClose={handleOnclose}/>
      ...
      ${open ? 'top-22 ':'hidden'}
       bg-neutral-200 lg:bg-neutral-900 lg:flex items-center lg:pb-0 pb-14 absolute lg:static lg:z-auto z-[-1] left-0 w-full lg:w-auto lg:pl-0 pl-24 transition-all duration-500`}>
        ...
        {/*Categories menu*/}
        {updown === true && visible === false &&<div className="hidden lg:flex p-2 pt-4 w-72 absolute right-80">
          <ul className=" bg-neutral-900 text-neutral-100 rounded shadow-lg overflow-y-auto">  
          {data.map((n)=>(
              <li key={n._id}>
                <a href={'/Category/'+n._id} ><div className="p-3 pr-10 pl-10 hover:text-teal-400 hover:bg-neutral-700 rounded duration-300 hover:shadow-xl">{n.name}</div> </a>
              </li>
            ))}
            <li>
